...
// @Accept json
// @Produce json
// @Param data query string true "Identificador dos dados a serem recuperados (ex.: data_highway_2022)" sendo os 4 últimos dígitos o ano a ser pesquisado, estando presente na base de dados os anos de 2018 a 2023.
// @Success 200 {array} string "Lista de canteiro central"
// @Router /highway [get]
func ListHighway(w http.ResponseWriter, r *http.Request) {
	...
	for accident, jsonData := range data {
		...
		if err != nil {
			http.Error(w, "Erro ao desserializar dados do highway "+accident, http.StatusInternalServerError)
			return
		}
