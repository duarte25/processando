...
// @Accept json
// @Produce json
// @Param data query string true "Identificador dos dados a serem recuperados (ex.: data_phase_day_2022)" sendo os 4 últimos dígitos o ano a ser pesquisado, estando presente na base de dados os anos de 2018 a 2023.
// @Success 200 {array} string "Lista de fase do dia"
// @Router /phase_day [get]
func ListPhaseDay(w http.ResponseWriter, r *http.Request) {
	...
	phaseDayDatas := make(map[string]entities.Accident)

	for phaseDay, jsonData := range data {
		var datas entities.Accident
		err := json.Unmarshal([]byte(jsonData), &datas)

		if err != nil {
			http.Error(w, "Error ao desserializar dados do phase day "+phaseDay, http.StatusInternalServerError)
			return
		}

		phaseDayDatas[phaseDay] = datas
	}

	jsonData, err := json.Marshal(phaseDayDatas)
	if err != nil {
